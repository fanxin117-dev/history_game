...
- **Backend**: Express.js 4.x + OpenAI-compatible SDK (calls Agnes API in dev, Anthropic Codex in production)
- **AI Provider**: Development = Agnes (`agnes-2.0-flash`, free, base URL `https://apihub.agnes-ai.com/v1`); Production = Anthropic Codex (comment toggle in `server/config/Codex.js`)
...
│   ├── config/Codex.js     ← AI provider config (Agnes dev / Codex prod)
│   ├── routes/game.js       ← 4 endpoints: /start, /question, /reveal, /end
│   ├── state/session-manager.js  ← GameSession CRUD, TTL cleanup
│   ├── services/Codex-service.js ← AI API wrapper (pickFigure, judgeQuestion, revealAnswer)
...
### AI API Pattern
- Uses OpenAI-compatible SDK (`openai` package) — works with both Agnes (dev) and Codex (prod)
- Switch provider by toggling comments in `server/config/Codex.js`
